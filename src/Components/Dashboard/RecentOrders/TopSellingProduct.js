...
const TopSellingProduct = ({ setFieldValue, values }) => {
    ...
    const { data, refetch, isLoading } = useQuery([product, values['filter_by']], () => request({ url: product, params: { status: 1, top_selling: 1, filter_by: values['filter_by']?.value ?? "this_year", paginate: 5 } }), { enabled: false, refetchOnWindowFocus: false, select: (data) => data.data.data });
