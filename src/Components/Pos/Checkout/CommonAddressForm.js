...
const CommonAddressForm = ({ type, updateId, addressMutate, loading, setModal }) => {
    const { data ,refetch ,isLoading } = useQuery([country], () => request({ url: country }), { enabled:false, refetchOnWindowFocus:false , select: (res) => res.data.map((country) => ({ id: country.id, name: country.name, state: country.state })) });
    useEffect(() => {
        isLoading && refetch()
      }, [isLoading])
