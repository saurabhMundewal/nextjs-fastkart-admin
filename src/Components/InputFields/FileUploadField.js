...
const FileUploadField = ({ values, updateId, setFieldValue, errors, multiple, loading, showImage,paramsProps, ...props }) => {
  ...
  return (
    <>
      ...
      {errors?.[props?.name] ? <ErrorMessage name={props.name} render={(msg) => <div className="invalid-feedback d-block">{t(handleModifier(storeImageObject).split(' ').join(""))} {t('IsRequired')}</div>} /> : null}
    </>
  );
};
