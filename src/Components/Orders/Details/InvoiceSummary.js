...
const InvoiceSummary = ({ data }) => {
  ...
  const moveToDelhivery = async (product) => {
    try {
      ...
      if (orderResponse) {
        const moveOrderResponse = await request({
          url: `${moveToDelhiveryAPI}/${product?.id}/${product?.is_oneDelhivery}`,
          method: "put",
        });
        ToastNotification(
          "success",
          "Order has been moved successfully. Please check the Shiprocket dashboard."
        );
        setAddedField(false);
        setTimeout(() => {
          router.push(`/order`);
      ...
      ToastNotification(
        "error",
        "Order is not moved, please try again",
        error.message
      );
    }
  };
      ...
      <CardBody>
        <div className="title-header">
          ...
          {data?.is_oneDelhivery === 0 ? (
            <Btn
              className="btn-animation btn-sm ms-auto btn-outline"
              onClick={() => setAddedField(true)}
            >
        ...
        {addedField && (
          <div className="tracking-total tracking-wrapper">
            <form>
              <div>
                <label htmlFor="parcelLength" className="mt-2">
                  Parcel Length:
                </label>
                <input
                  type="text"
              ...
              <div>
                <label htmlFor="parcelBreadth" className="mt-2">
                  Parcel Breadth:
                </label>
                <input
                  type="text"
              ...
              <div>
                <label htmlFor="parcelHeight" className="mt-2">
                  Parcel Height:
                </label>
                <input
                  type="text"
              ...
              <div>
                <label htmlFor="parcelWeight" className="mt-2">
                  ParcelWeight:
                </label>
                <input
                  type="text"
              ...
              <Btn
                className="btn-animation btn-sm ms-auto btn-outline mt-2 mb-2"
                onClick={() => handleSwitchDelhivery(data)}
              >
                Submit
              </Btn>
            </form>
        ...
        <div className="tracking-total tracking-wrapper">
          <ul>
            ...
            {!data?.is_digital_only ? (
              <li>
                {t("Shipping")} :
                <span>{convertCurrency(data?.shipping_total ?? 0)}</span>
              </li>
            ) : null}
            {data?.points_amount ? (
            ...
            {data?.coupon_total_discount !== 0 ? (
              <li className="txt-primary fw-bold">
                {t("discount")}{" "}
                <span>{convertCurrency(data?.coupon_total_discount)}</span>
              </li>
            ) : null}
            <li>
