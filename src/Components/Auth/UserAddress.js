...
const UserAddress = ({ values }) => {
  const { data  } = useQuery([country], () => request({ url: country }), {refetchOnWindowFocus: false, select: (res) => res.data.map((country) => ({ id: country.id, name: country.name, state: country.state })) });
