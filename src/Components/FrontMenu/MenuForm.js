...
const MenuForm = ({ mutate, updateId, loading }) => {
  ...
  const { data ,refetch:pageRefetch,isLoading:pageLoading } = useQuery([PagesAPI], () => request({ url: PagesAPI }), {enabled:false, refetchOnWindowFocus: false,select: (res) =>res?.data?.data?.map((elem) => {return { id: elem?.slug, name: elem?.title,slug:elem.slug };}),});
  useEffect(() => {
    updateId && refetch();
  }, [updateId]);
  useEffect(() => {
