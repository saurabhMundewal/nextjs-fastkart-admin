...
const FormShipping = ({ open, setActive, shippingData }) => {
  ...
  const { refetch,data, isLoading:countryLoader } = useQuery([country], () => request({ url: country }), {refetchOnWindowFocus: false, select: (res) => res.data.map((country) => ({ id: country.id, name: country.name })) });
