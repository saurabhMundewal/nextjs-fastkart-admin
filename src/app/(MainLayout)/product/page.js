...
const AllUsers = () => {
  ...
  const {data: brandData,isLoading: brandLoading,refetch: brandRefetch,} = useQuery([BrandAPI],() => request({url: BrandAPI, params: {status: 1},}),{ enabled: false,refetchOnWindowFocus: false,select: (res) =>res?.data?.data?.map((elem) => { return { id: elem.id, name: elem?.name, slug: elem?.slug };}),}
  );
  useEffect(() => {
    brandLoading && brandRefetch();
  }, [brandLoading]);

  const {data: storeData,isLoading,refetch: storeRefetch,} = useQuery([store],() => request({url: store,params: {status: 1,},}),
  ...
  const { data: categoryData, isLoading: categoryLoader } = useQuery([Category],() => request({ url: Category, params: { status: 1, type: "product" } }),{ refetchOnWindowFocus: false,select: (res) =>res?.data?.data.map((elem) => { return {id: elem.id,name: elem.name,image: elem?.category_icon?.original_url || placeHolderImage,slug: elem?.slug,subcategories: elem?.subcategories,};}),});
const productTypes =[
  {
    id: "physical",
